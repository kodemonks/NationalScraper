    ...
    while sucessfullySubmitted == False:
        ...
        try:
            driver.find_element_by_id(
                "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_dropOffLocation_searchCriteria").clear()
            driver.find_element_by_id(
                "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_dropOffLocation_searchCriteria").send_keys(
                returnLocation)
        except:
            driver.find_element_by_id(
                "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_returnToSameLocation").click()
            driver.find_element_by_id(
                "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_dropOffLocation_searchCriteria").clear()
            driver.find_element_by_id(
                "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_dropOffLocation_searchCriteria").send_keys(
        ...
        driver.find_element_by_id(
            "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_pickUpLocation_searchCriteria").clear()
        driver.find_element_by_id(
            "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_pickUpLocation_searchCriteria").send_keys(
            pickUpLocation)
        driver.execute_script(
            'document.getElementById("_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_pickUpDateTime_date").removeAttribute("readonly")')
        driver.find_element_by_id(
            "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_pickUpDateTime_date").clear()
        driver.find_element_by_id(
            "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_pickUpDateTime_date").send_keys(
            pickUpDate)
        driver.find_element_by_id(
            "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_pickUpDateTime_time").send_keys(
            pickUpTime)
        driver.execute_script(
            'document.getElementById("_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_dropOffDateTime_date").removeAttribute("readonly")')
        driver.find_element_by_id(
            "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_dropOffDateTime_date").clear()
        driver.find_element_by_id(
            "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_dropOffDateTime_date").send_keys(
            returnDate)
        driver.find_element_by_id(
            "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_dropOffDateTime_time").send_keys(
            returnTime)

        # Submits The Form
        try:
            driver.find_element_by_id(
                "_content_alamo_en_US_car_rental_home_jcr_content_reservationStart_insidersMember").send_keys(Keys.TAB)
...
if __name__ == "__main__":
    with open("input.csv", "rb") as f:
        ...
        writer.writerow(
            ["Name", "Total Price", "Price", "Pay Now & Save Total Price", "Pay Now & Save Price", "Pick Up Location",
             "Return Location", "Pick Up Date", "Return Date", "Pick Up Time", "Return Time"])
        wfile.close()
                        ...
                        for put in pickUpTimes:
                            for rt in returnTimes:
                                ...
                                for each in pageOffers:
                                    ...
                                    writer.writerow(
                                        [each.name, each.price, each.wprice, each.snprice, each.snwprice, pul, rl, pud,
                                         rd, put, rt])
                                wfile.close()
                                print('Done here!!\n')
        wfile.close()
